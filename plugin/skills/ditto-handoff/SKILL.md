...
1. **Link-pass:** call `figma_link_pass(projectId, figmaUrl)`.
   ...
   - **Guardrail — items must NOT be floating.** Linking a Figma frame is only "done" when each created item's copy actually lands under its frame in Ditto (the `connect` step persisted its instances). Check the result's `counts.floating` — it MUST be 0. The tool already resolves the true page and auto-retries the connect once, so a non-zero `floating` means the backend rejected those instances (the unofficial `connect` endpoint is unversioned and can silently drop them, returning 200 while saving nothing). Do NOT proceed as if the section is linked. Diagnose before continuing:
...
6. **Report once, at the end:** connected / created / renamed / variablised / translated / set-to-FINAL counts, ambiguous or skipped items, and any variables that need creating in the web app. Because the batch is already FINAL, call out anything you were unsure about so the user can spot-check it directly in Ditto.
