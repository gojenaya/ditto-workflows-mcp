...
4. **Translate — only if a variant was given.** Follow the `/ditto-translate` playbook scoped to this run's items: refresh assets, read `ditto://glossary/{variantId}` and the memory first (reuse existing translations), translate the untranslated touched items applying locked terms/voice, preserve `{{variables}}`, then `write_translations(batch, variantId, status: "WIP")`. Don't stop to confirm translations — write them; they'll be reviewed later.
